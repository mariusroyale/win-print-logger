...
@click.option("-s", "--string", default='', help="Event String", show_default=True)
@click.option("-d", "--data", default='', help="Event Data", show_default=True)
def main(app_name, event_id, category, event_string, event_data):
    ...
    DUMMY_EVT_ID = event_id  # Got this from another event
    DUMMY_EVT_CATEG = category
    DUMMY_EVT_STRS = [event_string]
    DUMMY_EVT_DATA = event_data  # b"Dummy event data"
